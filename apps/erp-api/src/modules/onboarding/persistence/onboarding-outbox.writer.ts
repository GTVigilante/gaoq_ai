...
export class OnboardingOutboxWriter {
  ...
  async append(event: OnboardingDomainEvent, session: ClientSession): Promise<void> {
    ...
    if (event.tenantId !== trusted.tenant.tenantId) throw new Error('入职 Outbox 拒绝跨租户事件');
    ...
    const envelope: CloudEvent<Record<string, unknown>> & { readonly schemaVersion: '1' } = {
      specversion: '1.0', id: eventId, source: '//gaoq-erp/onboarding-module',
      type: eventType, subject: `tenant/${event.tenantId}/onboarding/${event.aggregateId}`,
    ...
    await this.records.create([{
      eventId, tenantId: event.tenantId, aggregateType: 'onboarding.instance',
      aggregateId: event.aggregateId, aggregateVersion: event.version, eventType,
      envelope: { ...envelope }, status: 'pending', attempts: 0,
