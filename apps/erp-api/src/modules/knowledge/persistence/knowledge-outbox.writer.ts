...
export class KnowledgeOutboxWriter {
  ...
  async append(event: KnowledgeDomainEvent, session: ClientSession): Promise<void> {
    ...
    const envelope: CloudEvent<Record<string, unknown>> & { readonly schemaVersion: '1' } = {
      specversion: '1.0', id: eventId, source: '//gaoq-erp/knowledge-module',
      type: eventType, subject: `tenant/${event.tenantId}/knowledge/${event.aggregateId}`,
    ...
    await this.records.create([{
      eventId, tenantId: event.tenantId, aggregateType: 'knowledge',
