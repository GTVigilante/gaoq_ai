...
function assemble() {
  ...
  const service = new AttendanceApplicationService(
    ...
    approvals as never, rules as never, crypto as never, facts as never, corrections as never,
    snapshots as never, shiftPlans as never, sourceReadiness as never, outbox as never,
  );
  return {
    service, context, idempotency, profiles, employees, employments, approvals, rules, crypto,
...
describe('AttendanceApplicationService', () => {
  ...
  it('跨日班次必须先形成同规则版本的派生事实，并要求来源补拉至结束业务日', async () => {
    ...
    store.facts.findForRuleEvaluation.mockResolvedValue([{
      ...sourceFact(),
