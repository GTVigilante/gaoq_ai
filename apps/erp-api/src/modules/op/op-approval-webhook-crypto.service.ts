...
const keyRingSchema = z.object({
  ...
  keys: z.array(z.object({
    keyId: z.string().regex(ID), keyBase64url: z.string().regex(/^[A-Za-z0-9_-]{43}$/),
  ...
  if (new Set(ring.keys.map((key) => key.keyId)).size !== ring.keys.length) {
    context.addIssue({ code: 'custom', path: ['keys'], message: 'OP 审批密钥标识不得重复' });
  }
...
/** OP 审批原始请求专用 AES-256-GCM 密钥域，不复用经营摘要密钥。 */
@Injectable()
export class OpApprovalWebhookCryptoService {
  ...
  protect(tenantId: string, inboxId: string, rawBody: Buffer): ProtectedOpWebhook {
    ...
    const active = this.ring().keys.find((item) => item.status === 'active');
  ...
  unprotect(tenantId: string, inboxId: string, payload: ProtectedOpWebhook): Buffer {
    if (!ID.test(tenantId) || !ID.test(inboxId) || !ID.test(payload.payloadKeyId) ||
      ...
      !BASE64URL.test(payload.payloadAuthTag)) throw new Error('OP_APPROVAL_PAYLOAD_INVALID');
    ...
    if (iv.length !== 12 || tag.length !== 16 || ciphertext.length > OP_MAX_APPROVAL_BODY_BYTES) {
      throw new Error('OP_APPROVAL_PAYLOAD_INVALID');
    }
    const item = this.ring().keys.find((candidate) => candidate.keyId === payload.payloadKeyId);
    if (item === undefined) throw new Error('OP_APPROVAL_KEY_UNAVAILABLE');
    ...
    try {
      ...
      if (plaintext.length < 2 || plaintext.length > OP_MAX_APPROVAL_BODY_BYTES) {
        throw new Error('OP_APPROVAL_PAYLOAD_INVALID');
      }
      return plaintext;
    } catch (error) {
      if (error instanceof Error && error.message.startsWith('OP_APPROVAL_')) throw error;
      throw new Error('OP_APPROVAL_PAYLOAD_INVALID', { cause: error });
    } finally {
      master.fill(0);
      key.fill(0);
    }
  ...
  private ring(): z.infer<typeof keyRingSchema> {
    const raw = this.config.get('OP_APPROVAL_WEBHOOK_ENCRYPTION_KEYS', { infer: true });
  ...
  private assertInput(tenantId: string, inboxId: string, body: Buffer): void {
    if (!ID.test(tenantId) || !ID.test(inboxId) || body.length < 2 ||
      body.length > OP_MAX_APPROVAL_BODY_BYTES) throw new Error('OP_APPROVAL_PAYLOAD_INVALID');
  }
  ...
  private derive(master: Buffer): Buffer {
    return Buffer.from(hkdfSync(
      'sha256', master, Buffer.from('gaoq-op-approval-webhook-v1', 'utf8'),
      Buffer.from('payload-encryption', 'utf8'), 32,
    ));
  }

  private aad(tenantId: string, inboxId: string): Buffer {
    return Buffer.from(`op-approval-webhook\0${tenantId}\0${inboxId}`, 'utf8');
  }
}

function unavailable(): ServiceUnavailableException {
  return new ServiceUnavailableException({
    code: 'OP_APPROVAL_KEYRING_UNAVAILABLE', message: 'OP 审批回调加密设施暂不可用',
  });
}
