...
import { OpWebhookSecretResolver, type OpWebhookHeaders } from './op-webhook.service.js';
import {
...
export class OpApprovalWebhookService {
  ...
  async accept(
    ...
    if (rawBody !== undefined && rawBody.length > OP_MAX_APPROVAL_BODY_BYTES) {
      throw new PayloadTooLargeException({
        code: 'OP_APPROVAL_BODY_TOO_LARGE', message: 'OP 审批请求正文不得超过 1 MiB',
      });
    }
    if (rawBody === undefined || rawBody.length < 2) throw new BadRequestException({
      code: 'OP_APPROVAL_RAW_BODY_REQUIRED', message: 'OP 审批请求缺少有效原始正文',
    });
    ...
    const route = await this.routes.exists({
      tenantId: binding.tenantId, inboundClientId: clientId,
    ...
    const payloadHash = hashOpApprovalPayload(rawBody);
  ...
  private async reserveNonce(clientId: string, nonceHash: string, payloadHash: string): Promise<void> {
    try {
      const created = await this.redis.set(
        `op:webhook:approval:nonce:${clientId}:${nonceHash}`,
        payloadHash, 'EX', REPLAY_TTL_SECONDS, 'NX',
      );
      ...
      throw new ServiceUnavailableException({
        code: 'OP_APPROVAL_REPLAY_STORE_UNAVAILABLE', message: 'OP 审批防重放设施暂不可用',
      });
    }
  }

  private async enqueue(inboxId: string, tenantId: string, payloadHash: string): Promise<void> {
    await this.queue.add(OP_PROCESS_APPROVAL_REQUEST_JOB, { inboxId, tenantId }, {
      jobId: `op_approval_${payloadHash}`, attempts: 12,
...
function denied(): UnauthorizedException {
  return new UnauthorizedException({
    code: 'OP_APPROVAL_VERIFICATION_FAILED', message: 'OP 审批请求验证失败',
  });
}

function trace(rawBody: Buffer): string {
