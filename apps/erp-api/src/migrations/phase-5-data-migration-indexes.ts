...
export function buildPhaseFiveDataMigrationIndexManifest() {
  return buildIndexManifestFromSchemas([
    DataMigrationRunRecordSchema, DataMigrationItemRecordSchema, DataMigrationMappingRecordSchema,
    DataMigrationAssociationRecordSchema, DataMigrationAttachmentRecordSchema,
  ]);
}

export async function runPhaseFiveDataMigrationIndexMigration(
  connection: Parameters<typeof runAdditiveIndexMigration>[0],
  dryRun: boolean,
) {
  return runAdditiveIndexMigration(connection, dryRun, {
    migrationId: MIGRATION_ID, manifest: buildPhaseFiveDataMigrationIndexManifest(),
  });
}
...
async function main(): Promise<void> {
  ...
  if (args.some((arg) => arg !== '--dry-run')) {
    throw new Error('PHASE5_DATA_MIGRATION_INDEX_ARGUMENT_INVALID');
  }
  const uri = process.env.MONGODB_URI;
  if (uri === undefined || !uri.startsWith('mongodb://')) {
    throw new Error('PHASE5_DATA_MIGRATION_INDEX_MONGODB_URI_REQUIRED');
  }
  const connection = createConnection(uri, { autoIndex: false, serverSelectionTimeoutMS: 5_000 });
  try {
    await connection.asPromise();
    const result = await runPhaseFiveDataMigrationIndexMigration(
...
if (entryPath !== undefined && import.meta.url === pathToFileURL(entryPath).href) {
  void main().catch((error: unknown) => {
    const code = error instanceof Error &&
      /^PHASE5_DATA_MIGRATION_INDEX_[A-Z_]{1,96}$/.test(error.message)
      ? error.message : 'PHASE5_DATA_MIGRATION_INDEX_DATABASE_FAILURE';
