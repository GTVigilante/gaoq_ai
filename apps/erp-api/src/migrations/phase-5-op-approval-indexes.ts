...
const MIGRATION_ID = 'phase-5-op-approval-indexes-v1';

export function buildPhaseFiveOpApprovalIndexManifest() {
...
async function main(): Promise<void> {
  ...
  if (args.some((arg) => arg !== '--dry-run')) {
    throw new Error('PHASE5_OP_APPROVAL_INDEX_ARGUMENT_INVALID');
  }
  const uri = process.env.MONGODB_URI;
  if (uri === undefined || !uri.startsWith('mongodb://')) {
    throw new Error('PHASE5_OP_APPROVAL_INDEX_MONGODB_URI_REQUIRED');
  }
  const connection = createConnection(uri, { autoIndex: false, serverSelectionTimeoutMS: 5_000 });
  try {
    await connection.asPromise();
    const result = await runPhaseFiveOpApprovalIndexMigration(
...
if (entryPath !== undefined && import.meta.url === pathToFileURL(entryPath).href) {
  void main().catch((error: unknown) => {
    const code = error instanceof Error &&
      /^PHASE5_OP_APPROVAL_INDEX_[A-Z_]{1,96}$/.test(error.message)
      ? error.message : 'PHASE5_OP_APPROVAL_INDEX_DATABASE_FAILURE';
