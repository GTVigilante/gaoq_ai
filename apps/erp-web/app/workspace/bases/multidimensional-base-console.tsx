...
import type {
  BaseDefinition, BaseField, BaseFilterCondition, BaseRecordRow, BaseTable, BaseView,
  DatasetSchema, MultidimensionalBase, ResolvedDatasetRecord, ViewType,
} from './multidimensional-base-types';
import { baseTableId, datasetKey } from './multidimensional-base-types';
...
export function MultidimensionalBaseConsole() {
  ...
  const load = useCallback(async () => {
    setState('loading');
    try {
      const [baseResult, formResult, datasetResult] = await Promise.all([
        ...
        erpFetch<{ readonly items: readonly DatasetSchema[] }>('/api/datasets'),
      ]);
  ...
  useEffect(() => {
    const first = selectedBase?.tables.toSorted((left, right) => left.position - right.position)[0];
    const firstTable = first === undefined ? null : baseTableId(first);
    setSelectedTableId((current) => current !== null && selectedBase?.tables.some((table) => baseTableId(table) === current) ? current : firstTable);
    setSearch('');
  }, [selectedBase]);
  ...
  const selectedTable = selectedBase?.tables.find((table) => baseTableId(table) === selectedTableId) ?? null;
  const selectedView = availableViews.find((view) => view.id === selectedViewId) ?? null;
  const loadRecords = useCallback(async (table: BaseTable, view: BaseView | null) => {
    setRecordsState('loading');
    setRecordsNotice(null);
    try {
      if (table.kind === 'native') {
        const result = await erpFetch<{ readonly items: readonly BaseRecordRow[] }>(`/api/dynamic-forms/${table.formId}/records?limit=100`);
        setRecords(result.data.items);
      } else {
        const filters = (view?.config.filter?.conditions ?? []).flatMap((condition) =>
          condition.operator === 'eq' && condition.value !== undefined
            ? [{ fieldKey: condition.fieldKey, operator: 'eq' as const, value: condition.value }]
            : []);
        if (filters.length === 0) {
          setRecords([]);
          setRecordsNotice('外部数据集不会被整库复制。请给当前视图添加精确筛选条件后读取权威来源。');
          setRecordsState('ready');
          return;
        }
        const result = await erpFetch<{ readonly items: readonly ResolvedDatasetRecord[] }>('/api/datasets/records/query', {
          method: 'POST', headers: { 'content-type': 'application/json' },
          body: JSON.stringify({ dataset: table.dataset, filters, limit: 100 }),
        });
        setRecords(result.data.items.map((item) => Object.freeze({
          id: item.ref.recordId, version: item.ref.version, values: item.values, updatedAt: item.observedAt,
        })));
      }
      setRecordsState('ready');
    } catch { setRecords([]); setRecordsState('error'); }
  }, []);
  useEffect(() => { if (selectedTable === null) { setRecords([]); setRecordsState('idle'); } else void loadRecords(selectedTable, selectedView); }, [loadRecords, selectedTable, selectedView]);

  const definition = selectedTable?.kind === 'native' ? forms.find((form) => form.id === selectedTable.formId) ?? null : null;
  const externalSchema = selectedTable?.kind === 'external' ? datasets.find((dataset) => datasetKey(dataset.ref) === datasetKey(selectedTable.dataset)) ?? null : null;
  const fields = definition?.items.flatMap((item) => item.kind === 'field' ? [item.field] : []) ?? externalSchema?.fields.filter((field) => field.availability === 'generic') ?? [];
  ...
  return <main className="base-console" aria-labelledby="base-title">
    ...
    {state === 'ready' ? <section className="base-workbench">
      ...
      <div className="base-main">{selectedBase === null ? <Empty description="创建一个 Base，将已发布表单作为数据表接入" /> : <>
        ...
        <div className="base-table-tabs" role="tablist" aria-label="数据表">{selectedBase.tables.toSorted((left, right) => left.position - right.position).map((table) => { const id = baseTableId(table); return <button key={id} type="button" role="tab" aria-selected={id === selectedTableId} className={id === selectedTableId ? 'is-active' : ''} onClick={() => setSelectedTableId(id)}><TableOutlined /> {table.name}{table.kind === 'external' ? <Tag color="purple">外部</Tag> : null}<span>{id === selectedTableId ? records.length : ''}</span></button>; })}</div>
        ...
        {recordsNotice !== null ? <Alert type="info" showIcon message="外部数据集按引用读取" description={recordsNotice} /> : null}
    ...
    <CreateBaseDrawer open={createBaseOpen} datasets={datasets} onClose={() => setCreateBaseOpen(false)} onCreated={async () => { setCreateBaseOpen(false); await load(); }} />
    ...
    <CreateRecordDrawer open={createRecordOpen} definition={definition} onClose={() => setCreateRecordOpen(false)} onCreated={async () => { setCreateRecordOpen(false); if (selectedTable !== null) await loadRecords(selectedTable, selectedView); }} />
...
function CreateBaseDrawer(props: { readonly open: boolean; readonly datasets: readonly DatasetSchema[]; readonly onClose: () => void; readonly onCreated: () => Promise<void> }) {
  const [form] = Form.useForm<{ name: string; code: string; description: string; datasetKeys: string[] }>();
  const [saving, setSaving] = useState(false);
  const submit = async (values: { name: string; code: string; description?: string; datasetKeys: string[] }) => {
    setSaving(true);
    try {
      const selected: BaseTable[] = values.datasetKeys.map((key, position) => {
        const dataset = props.datasets.find((item) => datasetKey(item.ref) === key);
        if (dataset === undefined) throw new Error('DATASET_SELECTION_INVALID');
        return dataset.ref.kind === 'native'
          ? { kind: 'native', formId: dataset.ref.datasetId, name: dataset.name, primaryFieldKey: dataset.primaryFieldKey, position }
          : { kind: 'external', id: createUlid(), dataset: dataset.ref, name: dataset.name, primaryFieldKey: dataset.primaryFieldKey, position };
      });
      const views = selected.map((table) => ({ id: createUlid(), tableId: baseTableId(table), name: '默认表格', type: 'grid', config: { visibleFieldKeys: [], frozenFieldCount: 1, rowHeight: 'medium', sorts: [], groups: [] } }));
  ...
  return <Drawer title="新建多维 Base" open={props.open} onClose={props.onClose} className="base-create-drawer" destroyOnHidden><Typography.Paragraph>原生表单与 OP 等权威外部数据集可进入同一工作台；外部数据只保存引用，不复制为 GaoQ 记录。</Typography.Paragraph><Form form={form} layout="vertical" onFinish={(values) => { void submit(values); }} initialValues={{ datasetKeys: [], description: '' }}><Form.Item label="名称" name="name" rules={[{ required: true }, { min: 2, max: 128 }]}><Input placeholder="例如：招聘运营中心" /></Form.Item><Form.Item label="代码" name="code" rules={[{ required: true }, { pattern: /^[A-Za-z][A-Za-z0-9._-]{0,63}$/u }]}><Input placeholder="recruitment_ops" /></Form.Item><Form.Item label="说明" name="description"><Input.TextArea rows={3} maxLength={500} /></Form.Item><Form.Item label="接入业务数据集" name="datasetKeys" rules={[{ required: true, type: 'array', min: 1 }]}><Select mode="multiple" options={props.datasets.map((item) => ({ value: datasetKey(item.ref), label: `${item.ref.kind === 'external' ? `[${item.ref.system.toUpperCase()}]` : '[原生]'} ${item.name}` }))} placeholder="选择已授权数据集" /></Form.Item><Alert type="info" showIcon message="每张数据集自动建立默认视图；外部数据集必须通过精确筛选读取，写操作仍由权威系统控制。" /><Button className="base-create-submit" block type="primary" htmlType="submit" loading={saving}>创建 Base</Button></Form></Drawer>;
}
...
function BaseSettingsDrawer(props: { readonly open: boolean; readonly base: MultidimensionalBase | null; readonly onClose: () => void }) {
  return <Drawer title="权限、自动化与接入" open={props.open} onClose={props.onClose} width={520}>{props.base === null ? <Empty /> : <div className="base-settings"><Alert type="info" showIcon message="权限继承当前租户身份" description="Base、表定义、记录数据分别使用独立读取/设计/写入 Scope；敏感读取审计不可用时失败关闭。" /><section><header><strong>自动化</strong><Tag>{props.base.automations.length}</Tag></header>{props.base.automations.length === 0 ? <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="尚未配置自动化" /> : props.base.automations.map((automation) => { const adapterRequired = automation.actions.some((action) => action.type === 'notify' || action.type === 'connector_call'); return <article key={automation.id}><ThunderboltOutlined /><span><strong>{automation.name}</strong><small>{automation.trigger.type} · {automation.actions.length} 个动作{adapterRequired ? ' · 未登记动作将进入人工复核' : ''}</small></span><Tag color={!automation.enabled ? 'default' : adapterRequired ? 'gold' : 'green'}>{!automation.enabled ? '未启用' : adapterRequired ? '需适配器' : '运行中'}</Tag></article>; })}</section><section><header><strong>系统接入</strong></header><article><ApiOutlined /><span><strong>REST / OpenAPI</strong><small>外部系统按 Schema 与幂等键写入</small></span><Tag color="blue">可用</Tag></article><article><RobotOutlined /><span><strong>MCP</strong><small>复用应用服务的最小只读投影</small></span><Tag color="purple">原生</Tag></article><article><CodeOutlined /><span><strong>CLI</strong><small>批量导入与自动化运维入口</small></span><Tag color="cyan">原生</Tag></article></section><footer><code>{props.base.id}</code><span>版本 {props.base.version}</span></footer></div>}</Drawer>;
}
