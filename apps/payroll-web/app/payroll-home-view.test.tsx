...
describe('算薪首页服务端渲染', () => {
  it('未登录时提供 GaoQ ERP 登录入口', () => {
    ...
    expect(html).toContain('href="/payroll/api/auth/login"');
    expect(html).not.toContain('action="/payroll/api/auth/logout"');
  });
  ...
  it('已登录时只提供服务端退出表单', () => {
    ...
    expect(html).toContain('action="/payroll/api/auth/logout"');
    expect(html).not.toContain('href="/payroll/api/auth/login"');
  });
});
