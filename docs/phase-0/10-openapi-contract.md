...
它由 62 个 NestJS Controller 源文件确定性生成，覆盖全局 `/api` 前缀、四个
...
| 项目 | 数量 | 说明 |
|---|---:|---|
| Controller | 62 | 全部生产 Controller |
| Nest 路由声明 | 323 | `Get/Post/Put/Patch/Delete/All` |
| OpenAPI Path | 255 | 相同路径的不同方法合并 |
| OpenAPI Operation | 329 | MCP 的 `@All` 展开为七种标准方法 |
...
| class-validator DTO Schema | 162 | 字段、必填、类型、长度、范围、枚举、数组与继承 |
| 命名 Body Schema | 176 / 176 | 143 个 DTO、29 个运行时 Zod 登记、4 个编译器内联类型 |
| 成功响应 | 329 / 329 | 327 个显式内容 Schema、2 个 204 无正文 |
| Component Schema | 196 | `Problem`、162 个 DTO 与 33 个补充请求组件 |
...
当前生成器已扫描 162 个 class-validator DTO，生成字段级 JSON Schema
2020-12：必填/可选、原始类型、nullable union、继承、嵌套 DTO、长度/范围、
数组大小/唯一性、静态枚举、格式和正则来源均进入组件；143 个请求体通过 `$ref`
