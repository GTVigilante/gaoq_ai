...
运行时契约校验，只按可信身份摘要中的 `erp:org:master:write` 显示创建入口，并在
结果未知时冻结原正文和幂等键供原请求重试。真实浏览器弱网、实体角色矩阵和组织
管理员 UAT 仍待现场验收。

组织下发与招聘日历共用的外部身份解析已补齐严格调用参数、活动平台绑定和 bound
身份双标识最小投影校验；钉钉访问令牌的外部租户必须反向匹配活动绑定，
`unionId → userid` 结果只接受规范标识。20 项测试使目标文件四维均为 100%，
独立逐文件 90% 门禁已接入 `pnpm precheck`；标准 MCP 不新增外部身份、令牌或
平台写能力。
