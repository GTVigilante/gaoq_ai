...
4. 确认输出 `verified` 等于清单索引总数，并检查 `system_migration_runs/phase-1-indexes-v4` 为 `completed`。
...
- 当前清单覆盖审计、WORM 回执、身份、组织、集成、Outbox、幂等和首次开户共 19 个集合、64 个声明索引；v4 新增钉钉网页登录 `loginOpenId` 唯一部分索引。新增集合或索引必须新增迁移版本与测试。
