...
- 登录映射必须同时命中 provider、externalTenantId、unionId、平台登录标识和
  `status=bound`。钉钉通讯录 `userid` 与网页登录 `openId` 分属不同命名空间：
  首次可信扫码只允许按同一 `corpId + unionId` 原子登记 `loginOpenId`，后续扫码
  必须逐字匹配；不得用 `openId` 覆盖通讯录 `userid`。仓储只读取最小投影，随后
  复核持久化记录并返回冻结对象；不得返回 Mongoose 文档，不得按手机号、邮箱或
  显示名自动合并。
...
- 浏览器只保存 state 的 SHA-256 Base64URL 摘要；生产 Cookie 使用 `__Host-`、
  `HttpOnly`、`Secure`、`SameSite=Lax` 与根路径。回调拒绝超过 8 KiB 的 Cookie
  Header、重复同名 Cookie、非规范摘要和跨 Origin 请求，并在所有回调结果后清除
  state Cookie。Redis 只接受 43 字符规范 state 后再执行 GETDEL，故障返回稳定
  503，不把底层异常伪装成“状态不存在”。
...
- 人员 SSO 信任边界专项 13 个测试文件、142 项测试达到
  98.68%/97.66%/100%/99.39%，十五个生产文件逐文件四维均不得低于 90%。
