...
默认 `PAYROLL_SYSTEM_MODE=external` 时，`PayrollShadowService` 的导入、归因、
签署、控制摘要、行级差异和切换资格六类入口在可信主体/Scope 校验后、输入解释、
WebAuthn、幂等、Mongo 和解密前统一失败关闭。REST 与标准 MCP 复用同一服务，
迁移连接器或内部调用没有旁路。34 项专项测试达到
99.03%/97.24%/97.29%/98.88%（语句/分支/函数/行）。这项代码证据不构成两个
真实自然月的影子周期、独立签署或 Phase 6 Go/No-Go 证据。
