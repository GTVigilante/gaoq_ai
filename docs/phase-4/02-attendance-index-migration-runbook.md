...
迁移标识 `phase-4-attendance-indexes-v1` 固定源事实、修订和月结索引；`phase-4-attendance-rule-indexes-v2` 只追加班次计划及派生事实关联索引。两个迁移都只追加索引并写入带 manifest checksum 的迁移记录；已发布 v1 清单不得原地改写。

## 上线前检查

1. 备份四个 Attendance 集合与迁移记录集合，并确认 MongoDB Replica Set 健康。
...
3. 在影子数据上检查以下重复键；任一结果非空均为 No-Go：
   ...
   - 班次计划：`tenantId + sourcePlanBlindIndexes`；
   - 派生事实：同一非空 `tenantId + shiftPlanId` 只能一条。
4. 先执行构建，再运行：

```bash
pnpm --filter @gaoq/erp-api build
pnpm --filter @gaoq/erp-api migrate:phase4:attendance-indexes -- --dry-run
pnpm --filter @gaoq/erp-api migrate:phase4:attendance-rule-indexes -- --dry-run
```
...
pnpm --filter @gaoq/erp-api migrate:phase4:attendance-rule-indexes
```
...
- 创建跨日班次、处理上下班卡并计算后，派生事实唯一绑定班次；月结水位覆盖班次结束业务日，重复计算不产生第二条事实。
