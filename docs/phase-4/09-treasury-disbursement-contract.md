...
银行账号独立 AES-GCM/HMAC 盲索引、锁定工资到确定性文件、WORM、WebAuthn 三人分离批准、可信银行提交、隔离回盘 Inbox、恶意文件证据和 ERP 逐行复核已经落地。生产连接前仍必须完成：银行网关侧具体银行签名/加密配置与联调、失败行子批次、受控解冻、乱序回盘策略、税务文件、四方对账以及两个完整影子周期。MCP 永不接触这些能力。
