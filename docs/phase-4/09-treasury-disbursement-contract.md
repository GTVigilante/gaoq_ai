...
共享 `LegacyPayrollBoundaryService` 已覆盖全部 Treasury 应用服务。在线、迁移和
内部续跑方法各自校验可信主体及最小 Scope；`PAYROLL_SYSTEM_MODE=external`
时在强认证、生产授权、Payroll 端口、Mongo、加密、WORM、银行网关或 Return
Inbox 前失败关闭，不能依赖 Controller 调用顺序。银行账户、代发、回盘、失败
恢复和 Treasury 对账均有独立逐文件四维 90% 门禁并接入 `pnpm check`。这些
自动化证据不替代专业算薪、银行沙箱、WORM、真实历史迁移或财务 UAT。
