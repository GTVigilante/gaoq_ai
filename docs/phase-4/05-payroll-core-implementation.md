...
1. 从已发布规则包读取税率和基本减除费用，并复核 `rulesHash`。规则包法域作为加密计算输入进入 `inputHash`；法域代码用短横线表达父子层级，例如 `CN` 可覆盖 `CN-SH`，未声明层级关系的组合失败关闭。
2. 解密全部明确引用的已审批薪酬档案，逐一复核 `profileHash` 和密文/明文法域一致性。单档案必须覆盖完整工资月；多档案必须按生效顺序提交并恰好覆盖整月且无重叠，主档案必须是期间首段。
3. 多档案按自然日执行 `CALENDAR_DAY_HALF_UP` 分摊，冻结档案 ID/版本/哈希、法域、分配起止日和天数到 L4 输入快照。领域内核重新验证真实日历日、档案有效期、月初至月末连续覆盖和规则法域覆盖关系。税率包仍按整月累计预扣，法域差异的社保、公积金及薪酬组件来自各段已审批档案。
...
- 补发/冲销和年度汇算尚未实现；不得用普通工资运行或客户端金额绕过。
