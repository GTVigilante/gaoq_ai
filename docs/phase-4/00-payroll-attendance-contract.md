...
- Provider 的单条上下班卡分钟影响固定为零。受信任排班服务先追加版本化日班次，规则引擎必须等待“班次结束 + 晚退捕获窗口”后，按不重叠且端点不相接的闭区间生成唯一 `attendance_rules/shift` 派生事实。
- 同一班次只允许使用一个打卡 Provider；捕获窗口内出现多个 Provider 时失败关闭，禁止跨来源拼接上下班卡。派生事实的加密载荷必须固化算法版本、规则版本、打卡 Provider、上下班输入事实 ID 与计算结果，且该谱系必须进入月结快照哈希。
- 早到晚退不自动形成加班，缺卡先形成整班缺勤，再走审批修订。
