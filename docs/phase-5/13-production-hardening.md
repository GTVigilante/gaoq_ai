...
1000 并发只读 API、1000 人工资计算和三次可比证据的脚本与校验契约见[性能容量三次实测门禁](./15-performance-capacity-gate.md)。现场脱敏摘要只能由 `phase-5-performance` workflow policy 通过 GitHub Hosted Runner 和单次 OIDC 身份验收；工具交付不代替生产等价环境的三次真实执行。
...
当前 Bearer 仅保留五个精确指纹误报：SSO Client 已在网络调用前执行固定端点 allowlist；ISO 20022 生成器只处理 XML 文本节点，并执行 XML 1.0 字符范围和五实体编码，这两项复核到期日为 2026-10-20；两项是部署清单验证器中的 Kubernetes Secret 对象引用名称，不包含 Secret 数据或认证材料；一项是生产平台准入验证器主动注入敏感字段并断言拒绝的负向自测，不进入运行时配置、认证或外部请求，后三项复核到期日为 2026-10-23。五项均有回归测试；新增或变更指纹必须重新 CR，禁止通配或整条规则跳过。
