...
当前 Scope 的执行顺序固定为 `org_reference` → `org_workforce` → `org_employment`。每个 Scope 使用独立来源包、`sourceRunId`、控制总数和签署证据；劳动关系包内的 `employeeSourceId` 必须能在同一 `sourceSystem` 的员工映射中解析，禁止把多个 Scope 混入同一包。
