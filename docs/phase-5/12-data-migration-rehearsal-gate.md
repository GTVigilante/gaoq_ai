...
每轮必须按来源包手册完成全部已启用 Scope；`recruitment_reference` 位于审批 Scope 之后，且包内 HC 必须早于职位；`recruitment_candidates` 随后独立执行，并核验目标集合无身份明文、盲索引可精确检索且证据导出无 PII；`recruitment_applications` 再验证候选人/职位关联、阶段基线和 WORM 档案；`recruitment_interviews` 核验申请/员工关联、历史状态机版本、评价齐备、L3 密文和无日历副作用；`recruitment_offers` 最后核验运行中审批或终结历史引用、L4 条款密文、发送/决定/eSign 摘要、申请后续流水线版本及 WORM 档案，并证明没有普通审批、投递、签署或阶段事件副作用。任一 Scope 失败即整轮失败，不能用后续 Scope 的成功掩盖前置引用差异。
