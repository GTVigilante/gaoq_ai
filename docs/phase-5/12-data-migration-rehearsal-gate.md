...
每轮必须按来源包手册完成全部已启用 Scope；招聘 Scope 后依次执行三个考勤 Scope。源事实核验盲索引、时间线和密文；修订核验源事实/员工/approved 历史绑定；月结逐版核验截止集重算、控制总量、直接前序、重开审批 checksum、逐日密文、活动版本唯一及无普通关账/重开副作用。任一 Scope 失败即整轮失败，不能用后续 Scope 的成功掩盖前置引用差异。
