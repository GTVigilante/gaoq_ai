...
每轮必须按来源包手册完成全部已启用 Scope；招聘 Scope 依次核验参考、候选人、申请、面试与 Offer；随后 `attendance_source_facts` 核验员工映射、外部事件盲索引、严格时间线、业务日期、L4 密文、逐条 WORM 及无 Provider/修订/月结副作用。任一 Scope 失败即整轮失败，不能用后续 Scope 的成功掩盖前置引用差异。
