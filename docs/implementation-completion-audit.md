...
`pnpm --filter @gaoq/erp-api test:coverage`，321 个测试文件、1,974 项测试全部
通过；覆盖率为语句 82.43%、分支 75.33%、函数 85.27%、行 84.72%。分支仍需
覆盖 899 个分支才能达到 Phase 0 规定的全系统 80% 门槛，必须作为实施缺口继续处理，禁止
...
`pnpm quality:production-execution-authorization-coverage`。三十六条链路当前覆盖率分别为
...
100%/97.27%/100%/100% 和
100%/100%/100%/100%（语句/分支/函数/行）；三十六项阈值均固定为 90%，
使用相互隔离的报告目录，并已接入 `pnpm check`。这只证明三十六条关键链路达标，
