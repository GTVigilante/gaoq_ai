...
`pnpm --filter @gaoq/erp-api test:coverage`，408 个测试文件、6,676 项测试全部
通过。`vitest.config.ts` 已显式 `include: ['src/**/*.ts']`，因此测试未加载的
启动、Worker、Controller、迁移和适配器文件也进入分母；覆盖率为语句
92.80%（31,304/33,732）、分支 90.30%（21,574/23,889）、函数
93.52%（5,571/5,957）、行 93.93%（28,610/30,457）。全仓四维已达到 Phase 0
规定的 80% 门槛。全量命令通过
...
迁移能力。本证据不代表其余旧 Payroll/Treasury 内部入口已经全部下沉，也不替代
专业算薪、银行、真实历史审批、Replica Set 迁移演练和财务 UAT。
...
1. 按风险优先把共享旧系统边界继续下沉到其余 Payroll/Treasury Worker、迁移和
   内部应用服务，并补齐全仓和其余关键链路测试；达到章程阈值后再启用全仓不可
   回退门禁，并保留原始覆盖率报告。
