...
`pnpm --filter @gaoq/erp-api test:coverage`，323 个测试文件、2,727 项测试全部
通过；覆盖率为语句
87.01%（23,920/27,489）、分支 81.69%（15,822/19,367）、函数
89.05%（4,377/4,915）、行 88.73%（21,880/24,658）。全仓四维已达到 Phase 0
规定的 80% 门槛，分支高于最低值 328 个；必须保留专项 90% 门禁并继续增加
...
数据迁移控制面、数据迁移打包 CLI、Knowledge 考试运行 Relay、Knowledge 搜索索引 Relay、Knowledge REST 入口控制器、Knowledge 应用服务、Knowledge 考试应用与入口、Knowledge 领域模型、考勤应用、考勤仓储、考勤供应商拉取、考勤供应商入站处理、电子签回调处理、
...
`pnpm quality:production-execution-authorization-coverage`。六十二条链路当前覆盖率分别为
...
100%/100%/100%/100%、
100%/98.23%/100%/100% 和
100%/100%/100%/100%（语句/分支/函数/行）；六十二项阈值均固定为 90%，
使用相互隔离的报告目录，并已接入 `pnpm check`。这只证明六十二条关键链路达标，
