...
人才全周期仓储、招聘 Offer、Care 仓储、组织仓储、招聘仓储、知识库仓储、营销 CMS 和生产执行授权服务已建立
独立不可回退门禁：
...
`pnpm quality:production-execution-authorization-coverage`。三十七条链路当前覆盖率分别为
...
100%/97.27%/100%/100% 和
100%/100%/100%/100%（语句/分支/函数/行）；三十七项阈值均固定为 90%，
使用相互隔离的报告目录，并已接入 `pnpm check`。这只证明三十七条关键链路达标，
