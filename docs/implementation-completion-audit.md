...
`pnpm --filter @gaoq/erp-api test:coverage`，388 个测试文件、5,285 项测试全部
通过。`vitest.config.ts` 已显式 `include: ['src/**/*.ts']`，因此测试未加载的
启动、Worker、Controller、迁移和适配器文件也进入分母；覆盖率为语句
91.43%（29,458/32,216）、分支 88.80%（20,178/22,721）、函数
91.96%（5,255/5,714）、行 92.64%（26,899/29,033）。全仓四维已达到 Phase 0
规定的 80% 门槛。全量命令通过
