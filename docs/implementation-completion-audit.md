...
`gaoq.phase5.resilience.v3` 的两小时断连、自动追赶和对账门禁；韧性证据另绑定
...
容灾证据 v3 同时关闭七方演练批准只校验角色、证据 ID、意见摘要和时间、无法
...
payload 覆盖环境、commit、五类镜像、RPO/RTO、恢复/回滚、八域与消息对账、
九类外部连接、安全断言、产物摘要及七方批准元数据；每份签名再绑定共同摘要、
...
窗口。合法范围内数值的签后篡改、伪造签名、复用公钥、角色错配或 keyset 漂移
均由负向自测拒绝；Phase 6 Plan/Apply 重新绑定同一 keyset。仓库没有保存私钥，
