...
| 4 | 两个完整薪资周期影子计算；未解释差异为零 | 影子周期、版本化工资事实、专业算薪快照、审批、税务、银行文件、回盘和四方对账控制面已入库 | 两个真实完整周期、专业算薪/考勤/银行/税局联调和财务签署未完成；**No-Go** |
| 5 | 性能、安全、容灾、集成和全量迁移预验收通过 | 对应证据 schema、失败关闭校验器、受保护 workflow、独立 Ed25519 职责签名和 Go/No-Go 聚合门禁已入库 | 三轮生产等价运行、DAST/ASVS、外部沙箱、真实密钥和 UAT 未完成；**No-Go** |
| 6 | Go/No-Go 全满足；遗留系统只读归档 | 切换、平台准入、部署授权、回滚、Hypercare/归档证据契约及 Helm/Kubernetes 编排已入库 | 现场 Go/No-Go、三次演练、真实切换、连续四周运行和只读归档未发生；**No-Go** |

### 2.2 外部适配器字段映射

2026-07-30 已按 Phase 0 集成规范补齐
`apps/erp-api/src/modules/integration/adapters/`：钉钉、飞书、OP、e签宝、招聘
渠道、银行文件、税务文件和通知服务分别维护 canonical 字段、外部协议字段、权威
方向、幂等/回执和敏感性边界。`scripts/validate-docs.mjs` 会检查映射索引、九个
...
2026-07-30 补齐年度税局评估可信接入：公开制备 DTO 不再接受客户端
`officialAssessment`，受信任服务改由隔离税务网关读取并验证绑定租户、员工、
税年与请求摘要的 Ed25519 回执；员工本人仅可为自己的
`requires_employee_settlement` 记录创建五分钟内失效、官方同源的办理链接。
通用幂等账本只保存记录 ID、版本与摘要，重放从年度密文恢复，不保存 L4 税额；
REST 审计不保存一次性 URL，标准 MCP 未新增评估、链接、申报或收付能力。税务
HTTP 适配器 151 项测试通过，新适配器覆盖率为
93.75%/92.59%/100%/96.22%（语句/分支/函数/行）；年度/调整闭环 306 项测试
通过。真实税局沙箱、密钥轮换、断连追赶、金样例与薪税签署仍待现场验收。
