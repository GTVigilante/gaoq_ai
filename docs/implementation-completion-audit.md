...
`pnpm --filter @gaoq/erp-api test:coverage`，322 个测试文件、2,256 项测试全部
通过；覆盖率为语句 83.74%、分支 77.34%、函数 86.08%、行 85.87%。分支仍需
覆盖 512 个分支才能达到 Phase 0 规定的全系统 80% 门槛，必须作为实施缺口继续处理，禁止
...
`pnpm quality:production-execution-authorization-coverage`。四十四条链路当前覆盖率分别为
...
100%/97.27%/100%/100% 和
100%/100%/100%/100%（语句/分支/函数/行）；四十四项阈值均固定为 90%，
使用相互隔离的报告目录，并已接入 `pnpm check`。这只证明四十四条关键链路达标，
