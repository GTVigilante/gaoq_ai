...
`pnpm --filter @gaoq/erp-api test:coverage`，321 个测试文件、1,578 项测试全部
通过；覆盖率为语句 74.92%、分支 64.67%、函数 77.01%、行 77.99%。该结果低于
Phase 0 规定的全系统 80% 门槛，必须作为实施缺口继续处理，禁止通过排除生产文件、
降低阈值或只报告局部高覆盖率来宣称达标。

租户上下文、身份授权、审批数据加密、审批仓储、审批应用状态机、MCP 确认、
薪酬影子周期、Care 仓储和组织仓储已建立独立不可回退门禁：
...
`pnpm quality:approval-application-coverage`。九条链路当前覆盖率分别为
...
（语句/分支/函数/行）；九项阈值均固定为 90%，使用相互隔离的报告目录，并已
接入 `pnpm check`。这只证明九条关键链路达标，不替代全仓 80% 或其余关键服务
90% 的证据。
