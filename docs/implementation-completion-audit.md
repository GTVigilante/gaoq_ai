...
`pnpm --filter @gaoq/erp-api test:coverage`，408 个测试文件、6,694 项测试全部
通过。`vitest.config.ts` 已显式 `include: ['src/**/*.ts']`，因此测试未加载的
启动、Worker、Controller、迁移和适配器文件也进入分母；覆盖率为语句
92.90%（31,357/33,750）、分支 90.47%（21,618/23,894）、函数
93.53%（5,573/5,958）、行 94.01%（28,650/30,475）。全仓四维已达到 Phase 0
规定的 80% 门槛。全量命令通过
...
剩余下沉范围仅为旧 Payroll 算薪运行、审批、主数据、税务、对账和影子周期应用
服务；该仓库证据不替代真实外部联调、迁移演练和财务 UAT。
...
1. 按风险优先把共享旧系统边界继续下沉到其余旧 Payroll 算薪运行、审批、主数据、
   税务、对账和影子周期 Worker、迁移及内部应用服务，并补齐全仓和关键链路测试；
   达到章程阈值后再启用全仓不可回退门禁，并保留原始覆盖率报告。
