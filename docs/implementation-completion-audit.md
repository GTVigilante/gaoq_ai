...
`pnpm --filter @gaoq/erp-api test:coverage`，321 个测试文件、1,884 项测试全部
通过；覆盖率为语句 81.57%、分支 74.07%、函数 84.68%、行 84.06%。分支仍低于
...
`pnpm quality:approval-application-coverage`。二十八条链路当前覆盖率分别为
...
100%/97.27%/100%/100%（语句/分支/函数/行）；二十八项阈值均固定为 90%，
使用相互隔离的报告目录，并已接入 `pnpm check`。这只证明二十八条关键链路达标，
