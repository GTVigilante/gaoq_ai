...
`pnpm --filter @gaoq/erp-api test:coverage`，323 个测试文件、2,741 项测试全部
通过；覆盖率为语句
87.08%（23,946/27,496）、分支 81.89%（15,876/19,386）、函数
89.13%（4,383/4,917）、行 88.81%（21,905/24,664）。全仓四维已达到 Phase 0
规定的 80% 门槛，分支高于最低值 367 个；必须保留专项 90% 门禁并继续增加
...
Knowledge 持久化 Schema 已覆盖 21 项课程授权、可空考试配置、培训任务状态与
证据、考试运行状态机、超时、锁及搜索索引证据组合测试；无考试课程允许
`passingScoreBps=null`，受损任务记录在 Mongo 校验边界失败关闭。三个目标文件
合计覆盖率达到 100%/98.83%/100%/100%（语句/分支/函数/行），且各自均已
接入独立四维 90% 门禁。
