...
`pnpm --filter @gaoq/erp-api test:coverage`，325 个测试文件、3,003 项测试全部
通过。`vitest.config.ts` 已显式 `include: ['src/**/*.ts']`，因此测试未加载的
启动、Worker、Controller、迁移和适配器文件也进入分母；覆盖率为语句
84.79%（24,347/28,713）、分支 81.92%（16,200/19,773）、函数
85.88%（4,442/5,172）、行 86.51%（22,257/25,726）。全仓四维已达到 Phase 0
规定的 80% 门槛，分支高于最低命中数 381 个。全量命令通过
