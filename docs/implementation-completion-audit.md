...
|---|---|---|---|
| 0 | `docs/phase-0/`、Issue 模板、7 个 Milestone、标签及 Draft PR 流程 | Issue #41 的 GitHub Project 需要 `project` 权限；全仓覆盖率以 188 个分支余量达到 80%，其余关键链路 90% 证据尚未全部建立 | 未满足退出条件 |
...
`pnpm --filter @gaoq/erp-api test:coverage`，323 个测试文件、2,641 项测试全部
通过；覆盖率为语句
86.32%（23,722/27,480）、分支 80.97%（15,677/19,361）、函数
88.47%（4,347/4,913）、行 88.16%（21,733/24,650）。全仓四维已达到 Phase 0
规定的 80% 门槛，分支高于最低值 188 个；必须保留专项 90% 门禁并继续增加
...
`pnpm quality:production-execution-authorization-coverage`。五十八条链路当前覆盖率分别为
...
100%/100%/100%/100%、
100%/98.23%/100%/100% 和
100%/100%/100%/100%（语句/分支/函数/行）；五十八项阈值均固定为 90%，
使用相互隔离的报告目录，并已接入 `pnpm check`。这只证明五十八条关键链路达标，
...
Mongo 事务回调未执行时显式失败，自动重试的已回滚尝试不得残留待入队任务，达到
重试上限后指标归类为 `dead`；清理投递只允许可信 `system_job`。两个目标文件
合计覆盖率达到 100%/99.36%/100%/100%（语句/分支/函数/行），独立四维 90%
门禁已接入 `pnpm check`。
