...
2. `phase-6-deployment-plan` 隔离 Runner 使用只读集群身份，重新验收 Go/No-Go 原始证据，渲染并校验清单，检查外部 ConfigMap，执行 server-side dry-run 和 diff。
3. `phase-6-deployment-apply` 独立单次 Runner 只有在 `phase-6-production-deployment` Required Reviewers 批准后启动；它重新验收全部绑定、重新渲染并比对计划摘要，再执行 Helm 原子发布和只读 rollout 验证。
...
|---|---|---|---|
| `phase-6-production-plan` | `self-hosted,linux,x64,phase-6-deployment-plan` | 指定命名空间非敏感资源只读、dry-run、diff；禁止 `get/list/watch secrets`，不得更新资源 | 平台与安全负责人复核运行条件 |
| `phase-6-production-deployment` | `self-hosted,linux,x64,phase-6-deployment-apply` | 只允许管理本 release 的 namespaced 资源；禁止 `get/list/watch secrets`、ClusterRole 和云账号权限 | 至少两名独立 Required Reviewers，包含变更负责人和 SRE |
...
| `PHASE6_DEPLOYMENT_NAMESPACE` | 预创建且已启用 Restricted Pod Security 的命名空间 |
...
| `PHASE6_DEPLOYMENT_API_SECRET` | API 外部 Secret 名称，只检查 metadata 是否存在 |
| `PHASE6_DEPLOYMENT_WORKER_CONFIG_MAP` | Worker 外部 ConfigMap 名称 |
| `PHASE6_DEPLOYMENT_WORKER_SECRET` | Worker 外部 Secret 名称，只检查 metadata 是否存在 |
...
5. namespace 和两个非敏感 ConfigMap 存在，Secret 引用与平台证明一致，server-side dry-run 成功；diff 退出码只能为“无变化”或“存在计划变化”。
6. 渲染清单及其 `sha256`、脱敏 verdict 和 diff 保存 90 天，供 Required Reviewers 审阅。

执行阶段再次完成上述版本和清单检查，并要求重新渲染的摘要与计划 job output 完全一致。之后只运行：

```bash
helm upgrade --install <release> deploy/helm/gaoq-erp \
  --namespace <namespace> --values <只读values> \
...
截至文档交付时，GitHub 仓库没有配置上述 Environment 或 self-hosted Runner，因此没有执行任何生产计划或部署。要解除阻断，仓库所有者和平台负责人必须提供并批准：集群/命名空间、最小 RBAC、两类单次 Runner、完整变量、只读 values、离线 schema、Go/No-Go 原始证据和 Required Reviewers。缺少任一项时不得降低门禁或改用管理员 kubeconfig。
