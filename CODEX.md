...
- 2026-07-28 ERP API 覆盖率已修正为显式纳入 `src/**/*.ts` 全部生产源码，
  ...
  命令接入 `pnpm check`。当前真实基线为语句 89.00%（26,751/30,057）、
  分支 86.06%（17,813/20,696）、函数 89.43%（4,808/5,376）、行
  90.48%（24,442/27,013），355 个测试文件、3,870 项测试全部通过并达到
  全仓四维 80% 强制门槛。
  租户上下文、审计追加与 WORM 运输、审计链锚定、审计链验证、组织主数据应用、组织主数据入口、身份授权、
  ...
  校友授权清理协调、Attendance 规则纵切、Attendance 核心领域与 L4 加密、生产执行授权服务和 Phase 5 管理分析已建立八十三项
  90% 门禁，当前覆盖率基线集合为
