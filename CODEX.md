...
- 2026-07-28 ERP API 覆盖率已修正为显式纳入 `src/**/*.ts` 全部生产源码，
  ...
  命令接入 `pnpm check`。当前真实基线为语句 86.11%（24,781/28,776）、
  分支 82.46%（16,338/19,812）、函数 87.05%（4,507/5,177）、行
  87.91%（22,672/25,789），332 个测试文件、3,097 项测试全部通过并达到
  全仓四维 80% 强制门槛；分支高于最低命中数 488 个。
  租户上下文、审计链锚定、审计链锚定 Worker、审计链验证、组织主数据应用、组织主数据入口、身份授权、
