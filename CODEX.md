...
- 2026-07-28 ERP API 覆盖率已修正为显式纳入 `src/**/*.ts` 全部生产源码，
  ...
  命令接入 `pnpm check`。当前真实基线为语句 86.71%（25,072/28,913）、
  分支 83.01%（16,527/19,909）、函数 87.88%（4,572/5,202）、行
  88.49%（22,942/25,924），332 个测试文件、3,217 项测试全部通过并达到
  全仓四维 80% 强制门槛；分支高于最低命中数 599 个。
  ...
  知识库仓储、营销 CMS、营销入口与幂等核心、营销副作用可靠投递、OAuth 授权控制器、WebAuthn 强认证、入职应用与入口控制器、
