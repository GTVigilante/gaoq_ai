...
- 2026-07-27 已交付校友授权自动到期：稳定 BullMQ 延迟任务只携带可信租户与授权
  标识，Worker 使用最小 Scope 写入 `expired` 终态和脱敏事件；存量授权通过
  dry-run/apply 两阶段迁移按稳定 JobId 重建任务，不向 REST 或 MCP 暴露执行能力。
- 2026-07-27 已补齐校友授权自动到期：创建时使用可信租户与授权标识生成稳定
  BullMQ 延迟任务，到期 Worker 仅持有内部 expire Scope，发布脱敏事件并写审计；
  存量活动授权提供默认 dry-run、显式 apply 的批量任务重建工具。生日/周年关怀、
  下游个人数据删除证明和真实校友渠道仍未交付或待现场验收。
