...
- 2026-07-28 ERP API 覆盖率已修正为显式纳入 `src/**/*.ts` 全部生产源码，
  ...
  命令接入 `pnpm check`。当前真实基线为语句 87.28%（25,341/29,034）、
  分支 84.11%（16,828/20,005）、函数 88.11%（4,596/5,216）、行
  89.00%（23,178/26,042），335 个测试文件、3,326 项测试全部通过并达到
  全仓四维 80% 强制门槛；分支高于最低命中数 824 个。
