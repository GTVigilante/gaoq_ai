...
- 2026-07-28 ERP API 覆盖率已修正为显式纳入 `src/**/*.ts` 全部生产源码，
  ...
  命令接入 `pnpm check`。当前真实基线为语句 86.62%（25,015/28,877）、
  分支 82.80%（16,437/19,850）、函数 87.81%（4,564/5,197）、行
  88.42%（22,892/25,890），332 个测试文件、3,163 项测试全部通过并达到
  全仓四维 80% 强制门槛；分支高于最低命中数 557 个。
  ...
  知识库仓储、营销 CMS、营销入口与幂等核心、OAuth 授权控制器、WebAuthn 强认证、入职应用与入口控制器、
