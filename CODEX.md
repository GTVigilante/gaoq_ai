...
- 2026-07-28 ERP API 覆盖率已修正为显式纳入 `src/**/*.ts` 全部生产源码，
  ...
  命令接入 `pnpm check`。当前真实基线为语句 86.80%（25,123/28,943）、
  分支 83.15%（16,591/19,953）、函数 87.97%（4,579/5,205）、行
  88.57%（22,990/25,955），333 个测试文件、3,236 项测试全部通过并达到
  全仓四维 80% 强制门槛；分支高于最低命中数 628 个。
  ...
  知识库仓储、营销 CMS、营销入口与幂等核心、营销副作用可靠投递、审批通知可靠投递、OAuth 授权控制器、WebAuthn 强认证、入职应用与入口控制器、
  校友授权清理协调和生产执行授权服务已建立七十四项
  90% 门禁，当前分别为
