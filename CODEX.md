...
- 2026-07-28 ERP API 覆盖率已修正为显式纳入 `src/**/*.ts` 全部生产源码，
  ...
  命令接入 `pnpm check`。当前真实基线为语句 84.79%（24,347/28,713）、
  分支 81.92%（16,200/19,773）、函数 85.88%（4,442/5,172）、行
  86.51%（22,257/25,726），325 个测试文件、3,003 项测试全部通过并达到
  全仓四维 80% 强制门槛；分支高于最低命中数 381 个。
