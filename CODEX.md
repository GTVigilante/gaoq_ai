...
- 2026-07-28 ERP API 覆盖率已修正为显式纳入 `src/**/*.ts` 全部生产源码，
  ...
  命令接入 `pnpm check`。当前真实基线为语句 85.05%（24,444/28,739）、
  分支 82.07%（16,244/19,791）、函数 86.10%（4,454/5,173）、行
  86.79%（22,350/25,750），328 个测试文件、3,052 项测试全部通过并达到
  全仓四维 80% 强制门槛；分支高于最低命中数 411 个。
  ...
  薪酬审批、薪酬主数据、专业算薪主数据快照、薪酬四方对账、薪酬税务申报、薪酬 L4 数据加密、资金支付、Treasury 银行回盘、Treasury L4 数据加密、Treasury Outbox、Care 纪念日应用、Care 离职应用、数据迁移控制面、数据迁移打包 CLI、Knowledge 考试运行 Relay、
