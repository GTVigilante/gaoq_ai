...
- 2026-07-28 ERP API 覆盖率已修正为显式纳入 `src/**/*.ts` 全部生产源码，
  ...
  命令接入 `pnpm check`。当前真实基线为语句 89.54%（27,114/30,281）、
  分支 86.49%（18,046/20,863）、函数 89.95%（4,854/5,396）、行
  90.96%（24,767/27,226），366 个测试文件、4,011 项测试全部通过并达到
  全仓四维 80% 强制门槛。
...
- 身份用户会话与签名键轮换已覆盖 11 个测试文件、82 项测试。Refresh Token
  ...
  接入 `pnpm precheck`。
- 人员 SSO 信任边界已覆盖 10 个测试文件、99 项测试。钉钉、飞书、OP 回调逐层
  绑定 state、平台、外部租户与 ERP 租户，固定上游 HTTP 拒绝重定向、任意地址
  和超限响应；租户绑定、外部身份双标识映射与授权快照均从最小投影转换为经运行时
  完整性校验的冻结对象，跨租户或受损记录在令牌签发前失败关闭。十二个生产文件
  合计达到 99.45%/99.02%/100%/100%（语句/分支/函数/行），逐文件四维 90%
