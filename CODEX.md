...
- 2026-07-28 ERP API 覆盖率已修正为显式纳入 `src/**/*.ts` 全部生产源码，
  ...
  命令接入 `pnpm check`。当前真实基线为语句 89.40%（26,998/30,198）、
  分支 86.26%（17,939/20,796）、函数 89.85%（4,845/5,392）、行
  90.84%（24,661/27,147），364 个测试文件、3,953 项测试全部通过并达到
  全仓四维 80% 强制门槛。
  ...
  知识库仓储、营销 CMS、营销入口与幂等核心、营销副作用可靠投递、审批通知可靠投递、组织主数据外部投递可靠性、组织平台适配器安全边界、身份令牌与 OAuth 授权事务、身份用户会话与签名键轮换、OAuth Client Credentials 服务身份签发、OAuth 授权控制器、WebAuthn 强认证、入职应用与入口控制器、
  校友授权清理协调、Attendance 规则纵切、Attendance 核心领域与 L4 加密、生产执行授权服务和 Phase 5 管理分析已建立八十六项
  90% 门禁，当前覆盖率基线集合为
