...
- 2026-07-28 ERP API 全量覆盖率基线为语句 86.24%（23,693/27,472）、分支
  80.97%（15,675/19,359）、函数 88.27%（4,336/4,912）、行 88.07%
  （21,703/24,641），323 个测试文件、2,606 项测试全部通过并达到全仓四维
  80% 强制门槛；分支高于最低门槛 187 个，仍须通过专项门禁防止回退。
  租户上下文、审计链锚定、审计链验证、组织主数据应用、组织主数据入口、身份授权、
  审批数据加密、审批仓储、审批应用状态机、审批模板领域、MCP 确认、MCP 运行时、薪酬影子周期、薪酬运行、
