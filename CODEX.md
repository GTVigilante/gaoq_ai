...
- 2026-07-28 ERP API 覆盖率已修正为显式纳入 `src/**/*.ts` 全部生产源码，
  ...
  命令接入 `pnpm check`。当前真实基线为语句 87.48%（25,965/29,680）、
  分支 84.25%（17,172/20,380）、函数 88.29%（4,705/5,329）、行
  89.18%（23,767/26,648），344 个测试文件、3,413 项测试全部通过并达到
  全仓四维 80% 强制门槛；分支高于最低命中数 867 个。
  ...
  校友授权清理协调、Attendance 规则纵切和生产执行授权服务已建立七十七项
  90% 门禁，当前分别为
