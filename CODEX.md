...
- 2026-07-28 ERP API 覆盖率已修正为显式纳入 `src/**/*.ts` 全部生产源码，
  ...
  命令接入 `pnpm check`。当前真实基线为语句 89.99%（27,425/30,475）、
  分支 87.05%（18,324/21,048）、函数 90.48%（4,919/5,436）、行
  91.37%（25,039/27,402），373 个测试文件、4,116 项测试全部通过并达到
  全仓四维 80% 强制门槛。
  ...
  校友授权清理协调、Attendance 规则纵切、Attendance 核心领域与 L4 加密、生产执行授权服务和 Phase 5 管理分析已建立九十项
  90% 门禁，当前覆盖率基线集合为
