...
- 2026-07-28 ERP API 覆盖率已修正为显式纳入 `src/**/*.ts` 全部生产源码，
  ...
  命令接入 `pnpm check`。当前真实基线为语句 89.55%（27,138/30,303）、
  分支 86.52%（18,062/20,876）、函数 89.97%（4,856/5,397）、行
  90.97%（24,789/27,247），366 个测试文件、4,024 项测试全部通过并达到
  全仓四维 80% 强制门槛。
  ...
  校友授权清理协调、Attendance 规则纵切、Attendance 核心领域与 L4 加密、生产执行授权服务和 Phase 5 管理分析已建立八十七项
  90% 门禁，当前覆盖率基线集合为
