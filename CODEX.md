...
- 2026-07-28 ERP API 全量覆盖率基线为语句 87.72%（24,170/27,551）、分支
  82.45%（16,023/19,433）、函数 89.82%（4,424/4,925）、行 89.47%
  （22,115/24,717），323 个测试文件、2,788 项测试全部通过并达到全仓四维
  80% 强制门槛；分支高于最低门槛 476 个，仍须通过专项门禁防止回退。
  租户上下文、审计链锚定、审计链验证、组织主数据应用、组织主数据入口、身份授权、
  审批数据加密、审批仓储、审批应用状态机、审批入口控制器、审批模板领域、MCP 确认、MCP 运行时、MCP Tool 应用层、薪酬影子周期、薪酬运行、
