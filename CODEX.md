...
- 2026-07-28 ERP API 覆盖率已修正为显式纳入 `src/**/*.ts` 全部生产源码，
  ...
  命令接入 `pnpm check`。当前真实基线为语句 86.97%（25,252/29,034）、
  分支 83.47%（16,699/20,005）、函数 88.01%（4,591/5,216）、行
  88.69%（23,099/26,042），333 个测试文件、3,261 项测试全部通过并达到
  全仓四维 80% 强制门槛；分支高于最低命中数 695 个。
  ...
  知识库仓储、营销 CMS、营销入口与幂等核心、营销副作用可靠投递、审批通知可靠投递、组织主数据外部投递可靠性、OAuth 授权控制器、WebAuthn 强认证、入职应用与入口控制器、
  校友授权清理协调和生产执行授权服务已建立七十五项
  90% 门禁，当前分别为
