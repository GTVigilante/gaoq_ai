...
- 2026-07-28 ERP API 全量覆盖率基线为语句 87.82%（24,203/27,557）、分支
  82.65%（16,063/19,434）、函数 89.89%（4,428/4,926）、行 89.55%
  （22,140/24,723），323 个测试文件、2,819 项测试全部通过并达到全仓四维
  80% 强制门槛；分支高于最低门槛 515 个，仍须通过专项门禁防止回退。
  租户上下文、审计链锚定、审计链验证、组织主数据应用、组织主数据入口、身份授权、
  审批数据加密、审批仓储、审批应用状态机、审批入口控制器、审批模板领域、MCP 确认、MCP 运行时、MCP Tool 应用层、OP 审批桥入站申请、薪酬影子周期、薪酬运行、
