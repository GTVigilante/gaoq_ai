...
- 2026-07-27 ERP API 全量覆盖率基线为语句 85.40%（23,399/27,398）、分支
  ...
  租户上下文、审计链锚定、审计链验证、组织主数据应用、身份授权、OAuth
  授权控制器、
  ...
  知识库仓储、营销 CMS 和生产执行授权服务已建立五十三项 90% 门禁，当前分别为
...
- OAuth 授权控制器已覆盖 45 项预注册回调、PKCE、授权决策、授权码、
  `client_credentials`、协议错误与限流测试；Basic 客户端限流主体只取自
  Authorization，决策提交后的审计故障只作独立告警，不再改写业务终态。覆盖率
  达到 96.09%/91.51%/100%/99.45%（语句/分支/函数/行），独立 90% 门禁已
  接入 `pnpm check`。
