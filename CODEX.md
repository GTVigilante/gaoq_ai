...
- 2026-07-27 已交付 Knowledge 内容/题库校验与评分证据网关 Adapter：固定 HTTPS
  路由、独立凭据、Ed25519 回执验签、逐字段请求绑定、16 KiB 上限、5 秒超时及
  同幂等键单次重试；ERP 与 MCP 均不接收答卷或标准答案。真实网关、评分策略、
  主观题流程、超时自动交卷及培训 UAT 仍待外部验收。
- 2026-07-27 已补齐校友授权自动到期：创建时使用可信租户与授权标识生成稳定
  ...
  REST 或 MCP 暴露执行能力。生日/周年关怀、下游个人数据删除证明和真实校友渠道
  仍未交付或待现场验收。
