...
- 2026-07-28 ERP API 覆盖率已修正为显式纳入 `src/**/*.ts` 全部生产源码，
  ...
  命令接入 `pnpm check`。当前真实基线为语句 88.71%（26,547/29,924）、
  分支 85.75%（17,650/20,581）、函数 89.13%（4,784/5,367）、行
  90.22%（24,254/26,883），353 个测试文件、3,747 项测试全部通过并达到
  全仓四维 80% 强制门槛；分支高于最低命中数 1,185 个。
  ...
  校友授权清理协调、Attendance 规则纵切、Attendance 核心领域与 L4 加密、生产执行授权服务和 Phase 5 管理分析已建立八十二项
  90% 门禁，当前覆盖率基线集合为
