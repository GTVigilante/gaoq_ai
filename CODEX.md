...
- 2026-07-28 ERP API 覆盖率已修正为显式纳入 `src/**/*.ts` 全部生产源码，
  ...
  命令接入 `pnpm check`。当前真实基线为语句 88.38%（26,375/29,841）、
  分支 85.27%（17,488/20,507）、函数 89.00%（4,762/5,350）、行
  89.98%（24,119/26,803），352 个测试文件、3,643 项测试全部通过并达到
  全仓四维 80% 强制门槛；分支高于最低命中数 1,082 个。
  ...
  校友授权清理协调、Attendance 规则纵切、Attendance 核心领域与 L4 加密、生产执行授权服务和 Phase 5 管理分析已建立八十一项
  90% 门禁，当前覆盖率基线集合为
