...
- 2026-07-28 ERP API 覆盖率已修正为显式纳入 `src/**/*.ts` 全部生产源码，
  ...
  命令接入 `pnpm check`。当前真实基线为语句 90.02%（27,460/30,503）、
  分支 87.08%（18,345/21,065）、函数 90.49%（4,922/5,439）、行
  91.40%（25,070/27,428），373 个测试文件、4,183 项测试全部通过并达到
  全仓四维 80% 强制门槛。
  ...
  薪酬审批、薪酬主数据、专业算薪主数据快照、薪酬四方对账、薪酬税务申报、薪酬 L4 数据加密、资金支付、Treasury 银行提交出站边界、Treasury 银行回盘、Treasury L4 数据加密、Treasury Outbox、Phase 4 REST 入口、Care 纪念日应用、Care 离职应用、数据迁移控制面、数据迁移打包 CLI、Knowledge 考试运行 Relay、
  ...
  校友授权清理协调、Attendance 规则纵切、Attendance 核心领域与 L4 加密、生产执行授权服务和 Phase 5 管理分析已建立九十一项
  90% 门禁，当前覆盖率基线集合为
