...
- 2026-07-28 ERP API 覆盖率已修正为显式纳入 `src/**/*.ts` 全部生产源码，
  ...
  命令接入 `pnpm check`。当前真实基线为语句 86.05%（24,764/28,776）、
  分支 82.44%（16,334/19,812）、函数 86.98%（4,503/5,177）、行
  87.86%（22,658/25,788），331 个测试文件、3,091 项测试全部通过并达到
  全仓四维 80% 强制门槛；分支高于最低命中数 484 个。
