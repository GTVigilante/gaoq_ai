...
- 2026-07-28 ERP API 覆盖率已修正为显式纳入 `src/**/*.ts` 全部生产源码，
  ...
  命令接入 `pnpm check`。当前真实基线为语句 86.05%（24,755/28,767）、
  分支 82.43%（16,325/19,803）、函数 86.97%（4,502/5,176）、行
  87.85%（22,647/25,778），331 个测试文件、3,084 项测试全部通过并达到
  全仓四维 80% 强制门槛；分支高于最低命中数 482 个。
