...
- 2026-07-28 ERP API 覆盖率已修正为显式纳入 `src/**/*.ts` 全部生产源码，
  ...
  命令接入 `pnpm check`。当前真实基线为语句 86.32%（24,840/28,776）、
  分支 82.56%（16,357/19,812）、函数 87.32%（4,521/5,177）、行
  88.10%（22,722/25,789），333 个测试文件、3,122 项测试全部通过并达到
  全仓四维 80% 强制门槛；分支高于最低命中数 507 个。
  租户上下文、可观测性运行时、审计链锚定、审计链锚定 Worker、审计链验证、组织主数据应用、组织主数据入口、身份授权、
  ...
  知识库仓储、营销 CMS、OAuth 授权控制器、WebAuthn 强认证、入职应用与入口控制器、
  校友授权清理协调和生产执行授权服务已建立七十三项
  90% 门禁，当前分别为
...
- 可观测性运行时已覆盖 29 项独立 Bearer 鉴权、精确路由、无缓存响应、有界
  HTTP 指标标签、业务异常状态映射、队列固定状态轮询、重叠轮询跳过、采集
  失败恢复、Worker 健康检查及安全渲染失败响应测试；7 个生产文件合计覆盖率
  达到 100%/93.33%/100%/100%，逐文件四维 90% 门禁已接入 `pnpm check`。
- 审计链锚定 Worker 已覆盖 6 项固定任务名、批量上限、未知任务稳定错误码、
  WORM 停用不调度、六小时幂等调度、固定重试/退避/保留策略和失败传播测试；
  Processor 与 Scheduler 合计四维覆盖率均为 100%，独立 90% 门禁已直接接入
  `pnpm check`。
...
- Knowledge 应用服务已覆盖 41 项课程创建/发布/下架、任务分配/读取、员工主数据
  ...
  达到 99.59%/94.85%/98.21%/99.53%（语句/分支/函数/行），独立 90%
  门禁已直接接入 `pnpm check`。
...
- OP 审批结果回传已覆盖 74 项 Outbox 身份绑定、终态筛选、桥接版本单调性、
  ...
  数据身份到 Outbox 元数据，桥表通过精确旧版本乐观锁推进，
